...
int main()
{
    /* Don't really need any of this rubbish, but need to give it to the wallet
       instance */
    Logging::LoggerManager logManager;
    Logging::LoggerRef logger(logManager, "don't need this...");
    ...
    std::unique_ptr<CryptoNote::INode> node(
        new CryptoNote::NodeRpcProxy("don't need this...", 666,
                                     logger.getLogger()));
...
void crack(CryptoNote::WalletGreen &wallet)
{
    ...
    {
        std::cout << "Couldn't crack password" << std::endl;
    }
}

std::string getPrehashedPath()
{
    return "prehashed-passwords.txt";
}
